...
Каждое окружение хранит в `manifest.json` поле `environment` (test/beta/prod),
которое автоматически обновляется при продвижении. Файл `index.php` показывает
текущую версию и окружение в браузере.

### GitHub Webhook

На TEST-сервере доступен `githook.php` — при push в отслеживаемую ветку
GitHub автоматически обновляет тестовое окружение. На BETA и PROD этот файл
удаляется при развёртывании.
